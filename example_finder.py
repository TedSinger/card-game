...
def card_to_unicode(number, suit):
    ...
    number_offset = number + (number > 11) # the unicode symbol range includes chevaliers, which aren't in our deck
...
def card_to_svg(number, suit, x, y):
    ...
    rx = x
    ry = y + 4
    rect = f'<rect x="{rx}" y="{ry}" rx="5" ry="5" width="80" height="110" fill="white"/>'
    tx = x - 11
    ty = y + 94
